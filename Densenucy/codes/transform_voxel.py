...
def apply_gauss_and_convert_to_grid(coords, features, pdb, grid_resolution=1.0, max_dist=12.0):
    ...
    xi, yi, zi = get_bins(spacing=grid_resolution, expected_size=expected_size, pdb=pdb, xyz_min=xyz_min, xyz_max=xyz_max)
    if type(xi) == int and type(yi) == int and type(zi) == int:
        if xi == 0 and yi == 0 and zi == 0:
            return np.zeros(shape=(expected_size, expected_size, expected_size, num_features), dtype=np.float32), False
    ...
    return final_grid[0:expected_size, 0:expected_size, 0:expected_size, :], True


def get_bins(spacing, expected_size, pdb, xyz_min=None, xyz_max=None):
    xm, ym, zm = xyz_min
    xM, yM, zM = xyz_max
    ...
    #if len(yi)<25 or len(yi)>26 or len(xi)<25 or len(xi)>26 or len(zi)<25 or len(zi)>26:
    if len(yi)<20 or len(yi)>30 or len(xi)<20 or len(xi)>30 or len(zi)<20 or len(zi)>30:
        #print(f"-------------------------−−> anomaly {len(xi)=} {len(yi)=} {len(zi)=}")
        #print(f"{xm=}, {xM=}")
        #print(f"{ym=}, {yM=}")
        #print(f"{zm=}, {zM=}")
        #print(pdb)
        return(0,0,0) #try handling bad pockets
    else:
        return xi, yi, zi
