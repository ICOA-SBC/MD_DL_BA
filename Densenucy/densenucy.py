...
def load_model(cfg, dev):
    if cfg.network.pretrained_path == 'True':
        filename = os.path.join(cfg.experiment.model_path, f"densenucy_{cfg.experiment.run}.pth")
    else:
        filename = f"{cfg.network.pretrained_path.rstrip('.pth')}.pth"
    master_print(f"\tloading model {filename}")
    model = torch.load(filename, map_location=dev)
...
def save_model(model, pathname, experiment_name, run_name, rmse=''):
    if rmse:
        filename = os.path.join(pathname, f"densenucy_{run_name}_{rmse:.4f}.pth")
    else:
        filename = os.path.join(pathname, f"densenucy_{run_name}.pth")
...
def train(model, dataloaders, dataset_size, cfg, device):
    ...
    if cfg.network.pretrained_path:
        performance_pretrained_model = cfg.network.pretrained_path.rstrip('.pth').split('_')
        best_MSE, best_epoch=float(performance_pretrained_model[-1]), int(performance_pretrained_model[-2])
        print(f'pretrained model: epoch {best_epoch} with MSELoss {best_MSE}')
...
@hydra.main(config_path="./configs", config_name="densenucy")
def main(cfg: DictConfig) -> None:
    ...
    if cfg.training.only_test:
        ...
        batch_size = cfg.training.batch_size
        batch_size_per_gpu = batch_size // DIST.size
    ...
    if not cfg.training.only_test:
        ...
        train_sampler = DistributedSampler(train_dataset, num_replicas=DIST.size, rank=DIST.rank, shuffle=True, drop_last=True)
        val_sampler = DistributedSampler(valid_dataset, num_replicas=DIST.size, rank=DIST.rank, shuffle=False, drop_last=True)

        train_dataloader = DataLoader(train_dataset, batch_size=batch_size_per_gpu,
                                  shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, sampler=train_sampler)
        valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size_per_gpu,
                                  shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, sampler=val_sampler)
    ...
    with mlflow.start_run(run_name=cfg.mlflow.run_name) as run:
        ...
        if DIST.master_rank:
            ...
            DFresults.to_csv(f'../../../results/Densenucy_{cfg.mlflow.run_name}.csv')
            ...
            if not cfg.training.only_test:
                save_model(best_model, cfg.io.model_path, cfg.experiment_name, cfg.mlflow.run_name)
            ...
            if len(Lrun_name) == 4:
                grid.ax_joint.text(0.5, 11, f'Densenucy - R= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}\n{Lrun_name[2]}\n{Lrun_name[3]}', size=7)
            elif len(Lrun_name) == 3:
                grid.ax_joint.text(0.5, 11, f'Densenucy - R= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}\n{Lrun_name[2]}', size=7)
            elif len(Lrun_name) == 2:
                grid.ax_joint.text(0.5, 11, f'Densenucy - R= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}', size=7)
            elif len(Lrun_name) == 1:
                grid.ax_joint.text(0.5, 12.5, f'Densenucy - R= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}', size=7)
            ...
            grid.fig.savefig(f'../../../results/Densenucy_{cfg.mlflow.run_name}.pdf')
