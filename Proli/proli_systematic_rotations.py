...
def save_model(model, pathname, run_name, rmse=''):
    if rmse:
        filename = os.path.join(pathname, f"proli_rotations_{run_name}_{rmse:.4f}.pth")
    else:
        filename = os.path.join(pathname, f"proli_rotations_{run_name}.pth")
                ...
                if phase == 'val':
                    if rot == no_of_rotations - 1:
                        if epoch_metric < best_MSE:
                            ...
                            best_model_wts = copy.deepcopy(model.state_dict())
                            save_model(model, model_path, best_epoch, best_MSE)
                            patience = 0
                        else:
                            patience += 1
...
@hydra.main(config_path="./configs", config_name="proli_rotations")
def my_app(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run(run_name=cfg.mlflow.run_name) as run:
        ...
        DFresults.to_csv(f'../../../results/Proli_systematic-rotations_{cfg.mlflow.run_name}.csv')
        ...
        if not cfg.training.only_test:
            save_model(model, cfg.io.model_path, cfg.mlflow.run_name)
        ...
        if len(Lrun_name) == 4:
            grid.ax_joint.text(0.5, 11, f'Proli systematic rotations\nR= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}\n{Lrun_name[2]}\n{Lrun_name[3]}', size=7)
        elif len(Lrun_name) == 3:
            grid.ax_joint.text(0.5, 11, f'Proli systematic rotations\nR= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}\n{Lrun_name[2]}', size=7)
        elif len(Lrun_name) == 2:
            grid.ax_joint.text(0.5, 11, f'Proli systematic rotations\nR= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}\n{Lrun_name[1]}', size=7)
        elif len(Lrun_name) == 1:
            grid.ax_joint.text(0.5, 12.5, f'Proli systematic rotations\nR= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}', size=7)
        ...
        grid.fig.savefig(f'../../../results/Proli_systematic-rotations_{cfg.mlflow.run_name}.pdf')
