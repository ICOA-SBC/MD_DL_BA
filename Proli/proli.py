...
### UTILS
def save_model(model, pathname, run_name, rmse=''):
    if rmse:
        filename = os.path.join(pathname, f"proli_{run_name}_{rmse:.4f}.pth")
    else:
        filename = os.path.join(pathname, f"proli_{run_name}.pth")
...
def train(model, train_dataloader, valid_dataloader, dataset_size, cfg):
    ...
    for epoch in range(best_epoch, cfg_train.num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                if epoch_metric < best_MSE:
                    ...
                    save_model(model, cfg_model_path, best_epoch, best_MSE)
...
@hydra.main(config_path="./configs", config_name="proli")
def main(cfg: DictConfig) -> None:
    ...
    if not cfg.training.only_test:
        ...
        print(f"TMP: cfg.data.voxel={cfg.data.voxel}")
    ...
    with mlflow.start_run(run_name=cfg.mlflow.run_name) as run:
        ...
        DFresults.to_csv(f'../../../results/Proli_{cfg.mlflow.run_name}.csv')
        ...
        if not cfg.training.only_test:
            save_model(best_model, cfg.io.model_path, cfg.mlflow.run_name)
        ...
        if len(Lrun_name) == 4:
            ...
            grid.ax_joint.text(0.5, 12.5, f'Proli - R= {corr[0]:.2f} - RMSE= {rmse:.2f}\n{Lrun_name[0]}', size=7)
        ...
        grid.fig.savefig(f'../../../results/Proli_{cfg.mlflow.run_name}.pdf')
